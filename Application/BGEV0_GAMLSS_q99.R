...
#------------------------------------------------------------------------------
# BGEV distribution in GAMLSS with xi=0 - mu denotes quantile 0.99
#

BGEV0_q99 <- function(mu.link = "identity", sigma.link = "log", nu.link = "log"){
  ...
  structure(
    list(family = c("BGEV0_q99", "Bimodal Generalized Extreme Value distribution (Gumbel)"),
         ...
         dldm =  function(y, mu, sigma, nu){
           dldm = as.vector(attr(gamlss:::numeric.deriv(dBGEV0_q99(y, mu, sigma, nu, log = TRUE), "mu", delta = 1e-04), "gradient"))
           dldm
         },
         
         dldd = function(y, mu, sigma, nu){
           dldd = as.vector(attr(gamlss:::numeric.deriv(dBGEV0_q99(y, mu, sigma, nu, log = TRUE), "sigma", delta = 1e-04), "gradient"))
           dldd
         },
         
         dldv = function(y, mu, sigma, nu){
           dldv = as.vector(attr(gamlss:::numeric.deriv(dBGEV0_q99(y, mu, sigma, nu, log = TRUE), "nu", delta = 1e-04), "gradient"))
           dldv
         },
         ...
         #####
         G.dev.incr = function(y, mu, sigma, nu, ...) -2*dBGEV0_q99(y = y, mu = mu, sigma = sigma, nu=nu, log=TRUE),
         rqres = expression(rqres(pfun="pBGEV0_q99", type = "Continuous", y=y, mu=mu, sigma=sigma, nu=nu)),
...
dBGEV0_q99 <- function(y, mu = 1, sigma = 1, nu = 0.5, log = FALSE) {
 ...
 # Compute auxiliary variables:
  ...
  mub <- mu - (-sigma*log(-log(0.99)))^(1/(nu+1))
  y_mub <- y - mub
...
#-------------------------------------------------------------------------------
# Cumulative function
pBGEV0_q99 <- function(q, mu=1, sigma=1, nu=0.5, lower.tail = TRUE, log = FALSE){
  if (any(sigma <= 0)) warning("sigma must be positive")
...
#-------------------------------------------------------------------------------
# Quantile function
qBGEV0_q99 <- function(p, mu=1, sigma=1, nu=0.5, lower.tail = TRUE, log = FALSE){
  if (any(sigma <= 0)) warning("sigma must be positive")
...
rBGEV0_q99 <- function(n, mu=1, sigma=1, nu=0.5){
  if (any(sigma <= 0)) warning("sigma must be positive")
  if (any(nu <= 0)) warning("nu must be greater than 0")
  if (any(n <= 0)) stop(paste("n must be a positive integer", "\n", ""))
  
  U <- runif(n)
  rnumber <- qBGEV0_q99(U, mu, sigma, nu)
