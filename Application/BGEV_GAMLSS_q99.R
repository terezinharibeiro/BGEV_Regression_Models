...
#------------------------------------------------------------------------------
# BGEV distribution in GAMLSS - mu denotes quantile 0.99
#

BGEV_q99 <- function(mu.link = "identity", sigma.link = "log", nu.link = "identity", tau.link = "logit"){
  ...
  structure(
    list(family = c("BGEV", "Bimodal Generalized Extreme Value distribution"),
         ...
         dldm =  function(y, mu, sigma, nu, tau){
           dldm = as.vector(attr(gamlss:::numeric.deriv(dBGEV_q99(y, mu, sigma, nu, tau, log = TRUE), "mu", delta = 1e-04), "gradient"))
           dldm
         },
         
         dldd = function(y, mu, sigma, nu, tau){
           dldd = as.vector(attr(gamlss:::numeric.deriv(dBGEV_q99(y, mu, sigma, nu, tau, log = TRUE), "sigma", delta = 1e-04), "gradient"))
           dldd
         },
         
         dldv = function(y, mu, sigma, nu, tau){
           dldv = as.vector(attr(gamlss:::numeric.deriv(dBGEV_q99(y, mu, sigma, nu, tau, log = TRUE), "nu", delta = 1e-04), "gradient"))
           dldv
         },

           dldt = function(y, mu, sigma, nu, tau){
           dldt = as.vector(attr(gamlss:::numeric.deriv(dBGEV_q99(y, mu, sigma, nu, tau, log = TRUE), "tau", delta = 1e-04), "gradient"))
           dldt
         },
         ...
         G.dev.incr = function(y, mu, sigma, nu, tau, ...){
          -2*dBGEV_q99(y = y, mu = mu, sigma = sigma, nu=nu, tau=tau, log=TRUE)
         },
         rqres = expression(rqres(pfun="pBGEV_q99", type = "Continuous", y=y, mu=mu, sigma=sigma, nu=nu, tau=tau)),
...
dBGEV_q99 <- function(y, mu = 1, sigma = 1, nu = 0.5, tau = 0.5, log = FALSE) {
...
#-------------------------------------------------------------------------------
# Cumulative function
pBGEV_q99 <- function(q, mu=1, sigma=1, nu=0.5, tau=0.5, lower.tail = TRUE, log = FALSE){
  if (any(sigma <= 0)) warning("sigma must be positive")
  if (any(nu < -0.5 | nu > 1)) warning("If link.nu=own, nu must be >= -0.5 and <= 1")
  if (any(tau < 0)) warning("tau must be greater or equal to 0")
    
  # Compute auxiliary variables:
  mub <-  mu - sign((sigma/nu)*((-log(0.99))^(-nu) -1))*(abs((sigma/nu)*((-log(0.99))^(-nu) -1))^(1/(tau+1)))
...
#-------------------------------------------------------------------------------
# Quantile function
qBGEV_q99 <- function(p, mu=1, sigma=1, nu=0.5, tau=0.5, lower.tail = TRUE, log = FALSE){
  if (any(sigma <= 0)) warning("sigma must be positive")
  if (any(nu < -0.5 | nu > 1)) warning("If link.nu=own, nu must be >= -0.5 and <= 1")
...
rBGEV_q99 <- function(n, mu=1, sigma=1, nu=0.5, tau=0.5){
  if (any(sigma <= 0)) warning("sigma must be positive")
  if (any(nu < -0.5 | nu > 1)) warning("If link.nu=own, nu must be >= -0.5 and <= 1")
  if (any(tau < 0)) warning("tau must be greater than 0")
  ...
  U <- runif(n)
  mub <-  mu - sign((sigma/nu)*((-log(0.99))^(-nu) -1))*(abs((sigma/nu)*((-log(0.99))^(-nu) -1))^(1/(tau+1)))
  rnumber <- qBGEV_q99(U, mu, sigma, nu, tau)
