...
VSIGMA <- 1.0 #true sigma value
VDELTA <- 0.5 #true delta value

VTHETA <- c(VBETA, VSIGMA, VDELTA) # true theta values
VTHETA2 <- c(c(1.2, 2.0) , VSIGMA, VDELTA) # true theta values without beta3
...
thetahat <- matrix(NA, NREP, kk1 + 1)
se_thetahat <- matrix(NA, NREP, kk1 + 1)
...
mu.start = muStart, sigma.start = sigmaStart, nu.start = nuStart), error=function(e) {e})
  ...
  if(is.error(fit)){#in case of error or non convergence in the estimation procedure
      cont <- cont - 1
      f1 <- f1 + 1 
  }
  else
                  ...
                  if(fit$converged=="TRUE"){  #estimates only if there is convergence
                        ...
                        #***************parameter estimates*************************#
                         thetahat[cont,] <- c(as.numeric(coef(fit, what = "mu")),
                                              as.numeric(coef(fit, what = "sigma")),
                                              as.numeric(coef(fit, what = "nu")))
 
                        #***************asymptotic standard error estimates*************************#
                         se_thetahat[cont,] <- as.numeric(sqrt(diag(vcov(fit))))
...
cat(" TRUE VALUES ","\n")
cat(" Sample size ",N ,"\n")
out_VTHETA2 <- rbind(VTHETA2);colnames(out_VTHETA2)=c("beta1", "beta2", "sigma", "delta");rownames(out_VTHETA2)=c("");print(out_VTHETA2)
...
output_mle = rbind(out_estimates_MLE);colnames(output_mle)=c("Mean estimates","Bias","RMSE", "Mean SE");rownames(output_mle)=c("beta1", "beta2", "sigma", "delta");print(output_mle)
cat(" ========================================================================================== \n")
...
col.names = c("MLE_beta1", "MLE_beta2", "MLE_sigma", "MLE_delta"))

write.table(cbind(se_thetahat), 
file= paste(names_results2[l], ".txt", sep = ""), append = FALSE, sep = " ", dec = ".", row.names = FALSE,
col.names = c("se_MLE_beta1", "se_MLE_beta2", "se_MLE_sigma", "se_MLE_delta"))
