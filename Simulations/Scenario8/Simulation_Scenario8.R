...
sink("Results_Scenario8.txt")

names_results1 <- c("Estimates_Scenario8_n50", "Estimates_Scenario8_n100", "Estimates_Scenario8_n500")
names_results2 <- c("SE_Estimates_Scenario8_n50", "SE_Estimates_Scenario8_n100", "SE_Estimates_Scenario8_n500")
...
VTHETA2 <- c(c(1.2, 2.0), VSIGMA, VXI, VDELTA) # true theta values
...
X.esp <- model.matrix(~x1)
fitgev <-  suppressWarnings(tryCatch(vglm(y~x1, gevff(perc = NULL)), error=function(e) {e} )) #starting values via GEV regression
...
betascovhat <- estimates[4:(kk1+1)]
betahat <- c(beta1hat, betascovhat)
muStart <- as.vector(X.esp%*%betahat)
...
cat(" TRUE VALUES ","\n")
cat(" Sample size ",N ,"\n")
out_VTHETA2 <- rbind(VTHETA2);colnames(out_VTHETA2)=c("beta1", "beta2", "sigma", "xi", "delta");rownames(out_VTHETA2)=c("");print(out_VTHETA2)
...
output_mle = rbind(out_estimates_MLE);colnames(output_mle)=c("Mean estimates","Bias","RMSE", "Mean SE");rownames(output_mle)=c("beta1", "beta2", "sigma", "xi", "delta");print(output_mle)
cat(" ========================================================================================== \n")
...
col.names = c("MLE_beta1", "MLE_beta2", "MLE_sigma", "MLE_xi", "MLE_delta"))

write.table(cbind(se_thetahat), 
file= paste(names_results2[l], ".txt", sep = ""), append = FALSE, sep = " ", dec = ".", row.names = FALSE,
col.names = c("se_MLE_beta1", "se_MLE_beta2", "se_MLE_sigma", "se_MLE_xi", "se_MLE_delta"))
